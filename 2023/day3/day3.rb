...
puts special_chars(INPUT) # *$/=+@-%&#

data = parse_input(INPUT)

def get_adjacent_cells(data, x, y)
    [
        data.dig(y - 1, x - 1),
        data.dig(y - 1, x),
        data.dig(y - 1, x + 1),
...
def get_adjacent_cells_count(data, x, y)
    get_adjacent_cells(data, x, y).count { |cell| cell.match?(/[\*\$\/\=\+\@\-\%\&\#]/) }
end
...
def sum_of_number_with_adjacent_symbol(data)
    data.each_with_index.map do |line, y|
        line_numbers = []
        line.each_with_index do |cell, x|
            if is_number(cell) && get_adjacent_cells_count(data, x, y) > 0
                line_numbers << data[y][x-2 .. x+2].join.match(/[0-9]*#{cell}[0-9]*/)[0].to_i
                #puts "#{x}, #{y} => #{cell} / #{line}"
            end
        end
...
puts sum_of_number_with_adjacent_symbol(data)
